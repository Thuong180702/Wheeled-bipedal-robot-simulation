...
def test_equilibrium_reference_computation(controller):
    ...
    # Verify tau_wbc_equilibrium values > 5.0 on support joints
    assert np.all(controller.tau_wbc_equilibrium[SUPPORT_JOINTS] > 5.0), (
        f"Expected tau_wbc_equilibrium > 5.0 on support joints, got {controller.tau_wbc_equilibrium[SUPPORT_JOINTS]}"
    )
    ...
    # Verify equilibrium state values
    assert controller.equilibrium_state['com_z'] > 0.35, (
        f"Expected com_z > 0.35, got {controller.equilibrium_state['com_z']}"
    )
    assert abs(controller.equilibrium_state['pitch_x']) < 0.1, (
        f"Expected |pitch_x| < 0.1, got {controller.equilibrium_state['pitch_x']}"
    )
    assert abs(controller.equilibrium_state['roll_y']) < 0.1, (
        f"Expected |roll_y| < 0.1, got {controller.equilibrium_state['roll_y']}"
    )
    ...
    if controller.qfrc_constraint_ref is not None:
        assert controller.qfrc_constraint_ref.shape == (16,), (
            f"Expected qfrc_constraint_ref shape (16,), got {controller.qfrc_constraint_ref.shape}"
        )
...
def test_bias_cancellation_at_equilibrium(controller):
    ...
    assert np.allclose(tau_wbc_correction, 0.0, atol=0.5), (
        f"Expected near-zero correction at equilibrium, got {tau_wbc_correction}"
    )

    # Verify wrapped output matches static reference
    assert np.allclose(tau_wbc_wrapped, controller.tau_static_ref, atol=1.0), (
...
def test_correction_response_to_perturbations(controller):
    ...
    """
    # Build perturbed state with pitch perturbation
    current_state = {
        'com_z': controller.equilibrium_state['com_z'],
        'pitch_x': 0.05,  # 0.05 rad pitch perturbation
        'roll_y': controller.equilibrium_state['roll_y'],
        'joint_pos': controller.equilibrium_state['qpos'][7:17],
        'com_vel': np.zeros(3),
        'angular_vel': np.zeros(3),
    ...
    # Simulate WBC correction by adding [2.0, 3.0, 2.0, 3.0] to support joints
    tau_wbc_with_correction = controller.tau_wbc_equilibrium.copy()
    tau_wbc_with_correction[SUPPORT_JOINTS] += np.array([2.0, 3.0, 2.0, 3.0])

    # Call wrap() with corrected WBC torque
    tau_wbc_wrapped, telemetry = controller.wrap(
        tau_wbc_with_correction,
        current_state,
    )

    # Verify correction is nonzero for perturbation
    tau_wbc_correction = telemetry['tau_wbc_correction']
    correction_norm = np.linalg.norm(tau_wbc_correction)
    assert correction_norm > 1.0, (
        f"Expected correction > 1.0 Nm for pitch perturbation, got {correction_norm}"
    )
...
def test_no_mutation_of_live_data(mj_model, mj_data, mock_wbc_pipeline):
    ...
    - Original mj_data.qacc is unchanged after initialization
    - Original mj_data.ctrl is unchanged after initialization
    """
    # Store original state
    qpos_original = mj_data.qpos.copy()
    qvel_original = mj_data.qvel.copy()
    qacc_original = mj_data.qacc.copy()
    ctrl_original = mj_data.ctrl.copy()
    ...
    assert np.allclose(mj_data.qacc, qacc_original), (
        "mj_data.qacc was mutated during initialization"
    )
    assert np.allclose(mj_data.ctrl, ctrl_original), (
        "mj_data.ctrl was mutated during initialization"
    )
