#!/usr/bin/env python3
"""
N≥5 replication for ablation rows L2 and S1–S5 (R1 fix from Round 5 review).

Patches k2_jax_controller.py for each config, runs 8-direction push binary
search with N=5 independent reps, then restores the original file.

Each config: ~10 min. All 6: ~60 min.

Usage:
  .venv/bin/python scripts/replicate_ablation_n5.py
  .venv/bin/python scripts/replicate_ablation_n5.py --configs L2 S2 S3  # selective
"""
...
ci95_min = float(2.776 * f_min_std / np.sqrt(N_REPS))  # t-dist for N=5, df=4
ci95_med = float(2.776 * f_med_std / np.sqrt(N_REPS))
elapsed = time.time() - t0
...
CONFIGS = [
    ...
    # ── S1: Full ACC minus scheduled k_p ──
    {
        "id": "S1",
...
def main():
    ...
    print(f"N=5 reps × 8 dirs × 8 bisect iters per config")
    print(f"Est. time: ~{len(configs) * 10} min")
    print("=" * 72)
    ...
    for i, cfg in enumerate(configs):
        ...
        # Run measurement
        print(f"  Running push sweep (N=5)...", end=" ", flush=True)
        t0 = time.time()
        ...
        # Save incrementally
        out_path = OUT_DIR / "ablation_n5_partial.json"
    ...
    # ── Final save ──
    out_path = OUT_DIR / "ablation_n5_results.json"
    results["_metadata"] = {
        "n_reps": 5,
        "n_directions": 8,
    ...
    # ── Summary ──
    print(f"\n{'='*72}")
    print("N≥5 ABLATION RESULTS SUMMARY")
    print(f"{'='*72}")
