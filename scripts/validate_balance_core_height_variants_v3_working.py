...
class HeightVariantSetup:
    ...
    support_center_x: float = 0.0
    support_center_y: float = 0.0
    com_x_m: float = 0.0
    com_y_m: float = 0.0
    com_z_m: float = 0.0
    com_support_error_x: float = 0.0
    com_support_error_y: float = 0.0
    com_support_error_norm_xy: float = 0.0
    cp_x_m: float = 0.0
    cp_y_m: float = 0.0
    cp_error_x_m: float = 0.0
    cp_error_y_m: float = 0.0
...
def generate_height_variant_setup(
    ...
    if abs(pitch_x) > 0.1 or abs(roll_y) > 0.1:
        setup_valid = False
        failure_reasons.append(f"orientation_error")
...
def main():
    ...
    md_lines = [
        ...
        f"- Valid variants: {len(valid_variants)}/{len(variants)}",
    ...
    for s in setup_results:
        ...
        md_lines.extend([
            f"### {s.variant_name}",
            f"- Status: {status}",
            f"- Target: {s.target_com_z_m:.6f} m, Achieved: {s.achieved_com_z_m:.6f} m",
            f"- Method: {s.posture_search_method}",
            f"- Hip pitch: {s.hip_pitch_ref:.4f} rad, Knee: {s.knee_ref:.4f} rad",
            "",
        ])
