"""Generate boundary height setup JSONs for 0.300m and 0.480m CoM targets.

Uses the existing physical standing height envelope search infrastructure
to find the best statically feasible symmetric hip/knee posture for each
target height, then produces setup JSONs in the height-variant-setup format
expected by simulate_hierarchical_controller.py.

Usage:
    python scripts/generate_boundary_height_setups.py
    python scripts/generate_boundary_height_setups.py --output-dir outputs/custom
"""
...
def build_height_variant_setup(
    *,
    ...
    Sets the MuJoCo state from the candidate, calibrates root_z,
    runs forward kinematics, and computes all fields needed by
    simulate_hierarchical_controller.py.
    """
    # Set joint positions from candidate
    data.qpos[:] = 0.0
    ...
    # Equilibrium joint positions (qpos[7:17])
    ...
    result = evaluate_static_standing_pose(
        ...
        candidate_source="boundary_target_search",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate boundary height setup JSONs for 0.300m and 0.480m CoM targets"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    print("=" * 80)
    print("Boundary Height Setup Generator")
    print("=" * 80)
    ...
    for target_info in BOUNDARY_TARGETS:
        ...
        print(f"  Pitch: {setup['pitch_x_rad']:.6f} rad")
        print(f"  Roll: {setup['roll_y_rad']:.6f} rad")
        print(f"  Yaw: {setup['yaw_z_rad']:.6f} rad")
    ...
    # Write static validation summary
    validation_path = output_dir / "static_validation_summary.json"
    validation_path.write_text(json.dumps(static_validations, indent=2), encoding="utf-8")

    # Write report
    report_lines = [
        "# Boundary Height Setup Report",
        "",
        f"**Targets:** {', '.join(t['name'] + '=' + str(t['target_com_z_m']) + 'm' for t in BOUNDARY_TARGETS)}",
    ...
    for v in static_validations:
        ...
        report_lines.extend([
            ...
            f"- Pitch: {v['pitch_x_rad']:.6f} rad",
            f"- Roll: {v['roll_y_rad']:.6f} rad",
            f"- Yaw: {v['yaw_z_rad']:.6f} rad",
            f"- Joint limit margin: {v['joint_limit_margin_rad']:.4f} rad",
            f"- Root-z-only: {v['candidate_is_root_z_only']}",
            "",
        ])

    report_path = output_dir / "physical_target_height_setup_report.md"
    report_path.write_text("\n".join(report_lines), encoding="utf-8")

    print("\n" + "=" * 80)
    if all_valid:
        print("All boundary height setups PASSED static validation")
        print(f"Artifacts written to: {output_dir}")
    else:
        print("WARNING: Some boundary height setups FAILED static validation")
        print("Do not run dynamic validation until static issues are resolved")
    print("=" * 80)
