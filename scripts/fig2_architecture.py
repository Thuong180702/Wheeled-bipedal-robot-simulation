#!/usr/bin/env python3
"""Fig.2: ACC Two-Channel Torque Assembly — clean layout, no overlap, uniform boxes.

Emits the colour and the greyscale variant from this one source; the two used to
be separate files and drifted apart (the colour one still showed the pre-override
wheel channel).
"""
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import textwrap

# Repository root, resolved from this file so the script runs from any checkout.
from pathlib import Path as _Path
_ROOT = _Path(__file__).resolve().parent.parent

plt.rcParams.update({
    'font.family': 'serif', 'font.size': 6.5,
    'figure.dpi': 300, 'savefig.bbox': 'tight', 'savefig.pad_inches': 0.0,
})
OUT_DIR = str(_ROOT / 'paper' / 'figures') + '/'

PALETTES = {
    'acc_architecture': dict(
        wheel_fc='#dbe5f1', leg_fc='#d9ead3', out_fc='#b6d7a8',
        cond_fc='#fff2cc', hdr_l='#1f4e79', hdr_r='#38761d', tc='#000'),
    'acc_architecture_bw': dict(
        wheel_fc='#ececec', leg_fc='#ececec', out_fc='#c4c4c4',
        cond_fc='#ffffff', hdr_l='#000', hdr_r='#000', tc='#000'),
}

# ─── Layout ───
XL, XR, XC = 2.0, 8.0, 5.0             # left col, right col, centre divider
BW   = 3.5                               # uniform box width
BFS  = 4.9                               # box content fontsize
GAP  = 1.1                               # vertical gap between boxes
OUT_H = 0.5                              # output box height
OUT_W = 3.6                              # output box width (text is long; 4.0
                                         # overhangs the axes and gets clipped)
OUT_FS = 4.6                             # output box fontsize

# ─── Box texts (pre-wrapped, uniform line count drives the box height) ───
# Wheel channel carries sagittal balance only: lateral and yaw actuate on the
# hip-roll / hip-yaw pairs, i.e. in the leg-joint channel.
BOX_TEXTS = {
    'balance':  'τ_balance\nSagittal Balance Core\n(pitch PD + vel. damping\n + position P, per-wheel damping)\nalways active',
    'anchor':   'g_anchor · τ_anchor\nProximity-Gated Anchor\n(integral + damping boost)\ngated by g_prox, g_env, g_θ',
    'flight':   'τ_flight   (override)\nContact-Loss Recovery\n(reaction-wheel attitude PD)\nblended in by g_flight',
    'posture':  'τ_posture(hᴸ_cmd, hᴿ_cmd)\nPosture & Height Regulation\n(Jacobian PD + hip-yaw\ndivergence + homing; per-leg\nground split via g_terrain)',
    'latyaw':   'τ_lat + τ_yaw\nLateral & Heading Regulation\n(antisym. hip-roll / hip-yaw\npairs, saturated)\nalways active',
}

max_lines = max(t.count('\n') + 1 for t in BOX_TEXTS.values())

# Box height: max_lines * line_height + padding
LINE_H = 0.13                         # per-line height in data coords at BFS
BH = max_lines * LINE_H + 0.28        # uniform box height

# ─── Y-layout: bottom-up ───
Y_OUT    = 0.6
Y_COMP3  = Y_OUT + OUT_H + GAP
Y_PLUS2  = Y_COMP3 + BH + GAP/2
Y_COMP2  = Y_COMP3 + BH + GAP
Y_PLUS1  = Y_COMP2 + BH + GAP/2
Y_COMP1  = Y_COMP2 + BH + GAP
Y_HEAD   = Y_COMP1 + BH + 0.50
Y_COND   = Y_HEAD + 0.6
Y_TITLE  = Y_COND + 1.0

# Bottom section — gate boxes are stacked dynamically below, since their height
# depends on how many lines the definition text wraps to.
Y_GATE_HDR = Y_OUT - 0.45

Y_MAX = Y_TITLE + 0.4

GATE_FS = 4.4           # gate-definition font size


def build(name, pal):
    # ═══════════════════════════════════════════════════
    # SETUP
    # ═══════════════════════════════════════════════════
    fig, ax = plt.subplots(figsize=(3.45, 3.1))
    ax.set_xlim(0, 10); ax.axis('off')
    # The axes fills the figure: the channel headers and output boxes overhang the
    # data range, and a default-inset axes crops them out of the tight bbox.
    fig.subplots_adjust(left=0.03, right=0.97, bottom=0.01, top=0.99)

    # ═══════════════════════════════════════════════════
    # HELPERS
    # ═══════════════════════════════════════════════════
    gate_items = []   # (text, rect) pairs; every box is checked for text containment
    wrap_cache = {}

    def fit_text(t, rect, pad_px=4):
        """Shrink a text until it fits inside its own box. Wrapping the multi-line
        component labels would break their line structure, so scale the font instead."""
        for _ in range(12):
            fig.canvas.draw()
            tb = t.get_window_extent(fig.canvas.get_renderer())
            rb = rect.get_window_extent(fig.canvas.get_renderer())
            over = max(tb.width - (rb.width - 2*pad_px), tb.height - (rb.height - 2*pad_px))
            if over <= 0:
                return
            t.set_fontsize(t.get_fontsize() * 0.94)

    def draw_box(x, y, txt, fc, fw='bold'):
        """Uniform box with pre-wrapped text centred inside."""
        rect = mpatches.FancyBboxPatch(
            (x - BW/2, y), BW, BH,
            boxstyle='round,pad=0.2', facecolor=fc,
            edgecolor='#555', linewidth=0.6, zorder=5)
        ax.add_patch(rect)
        t = ax.text(x, y + BH/2, txt, ha='center', va='center',
                    fontsize=BFS, fontweight=fw, color=pal['tc'], zorder=6)
        fit_text(t, rect)
        gate_items.append((t, rect))

    def draw_out(x, y, txt):
        """Shorter output box."""
        rect = mpatches.FancyBboxPatch(
            (x - OUT_W/2, y), OUT_W, OUT_H,
            boxstyle='round,pad=0.12', facecolor=pal['out_fc'],
            edgecolor='#555', linewidth=0.6, zorder=5)
        ax.add_patch(rect)
        t = ax.text(x, y + OUT_H/2, txt, ha='center', va='center',
                    fontsize=OUT_FS, fontweight='bold', color=pal['tc'], zorder=6)
        fit_text(t, rect)
        gate_items.append((t, rect))

    def plus(x, y):
        ax.text(x, y, '+', fontsize=11, fontweight='bold', color='#555',
                va='center', ha='center', zorder=8)

    def blend(x, y):
        """Flight is an override, not a summand — mark the junction as a blend."""
        ax.text(x, y, '⊕  convex blend, not a sum', fontsize=4.2, style='italic',
                color='#555', va='center', ha='center', zorder=8)

    def arrow(x, yt, yb):
        ax.annotate('', xy=(x, yb), xytext=(x, yt),
                    arrowprops=dict(arrowstyle='->', color='#777', lw=1.0), zorder=4)

    def gate_wrap(txt):
        """Chars per line that actually fit the 9.6-wide box, measured not guessed."""
        if txt not in wrap_cache:
            probe = ax.text(0, 0, txt, fontsize=GATE_FS, alpha=0)
            fig.canvas.draw()
            w_px = probe.get_window_extent(fig.canvas.get_renderer()).width
            probe.remove()
            box_px = 9.2 / (ax.get_xlim()[1] - ax.get_xlim()[0]) * ax.bbox.width
            wrap_cache[txt] = max(20, int(len(txt) * box_px / w_px))
        return wrap_cache[txt]

    def gate_box(x, y, txt, fc):
        """Draw a full-width definition box with its BOTTOM edge at y."""
        lines = textwrap.wrap(txt, width=gate_wrap(txt))
        gh = 0.2 + 0.13 * len(lines)
        rect = mpatches.FancyBboxPatch(
            (x - 4.8, y), 9.6, gh,
            boxstyle='round,pad=0.1', facecolor=fc,
            edgecolor='#ccc', linewidth=0.4, zorder=5)
        ax.add_patch(rect)
        t = ax.text(x, y + gh/2, '\n'.join(lines), ha='center', va='center',
                    fontsize=GATE_FS, color='#444', zorder=6)
        gate_items.append((t, rect))
        return gh

    # ═══════════════════════════════════════════════════
    # TITLE + PHYSICAL CONDITIONS
    # ═══════════════════════════════════════════════════
    ax.text(XC, Y_TITLE, 'ACC Two-Channel Torque Assembly',
            ha='center', fontsize=9, fontweight='bold')

    ax.text(XC, Y_COND + 0.55, 'Physical Conditions  —  Smoothstep Gate Inputs',
            ha='center', fontsize=6.5, fontweight='bold', color='#555')

    cond = ('Proximity |Δx|  ·  Quietness EMA(|v|)  ·  Stability θ, θ̇  ·  '
            'Contact Fzᴸ, Fzᴿ  ·  Terrain hᴸ_gnd, hᴿ_gnd')
    gate_box(XC, Y_COND - 0.1, cond, pal['cond_fc'])

    # ═══════════════════════════════════════════════════
    # CHANNEL HEADERS
    # ═══════════════════════════════════════════════════
    hdr_l = ax.text(XL, Y_HEAD, 'Wheel Torque  τ_w', ha='center',
                    fontsize=7.5, fontweight='bold', color=pal['hdr_l'])
    hdr_r = ax.text(XR, Y_HEAD, 'Leg-Joint Torque  τ_q', ha='center',
                    fontsize=7.5, fontweight='bold', color=pal['hdr_r'])

    # ═══════════════════════════════════════════════════
    # WHEEL CHANNEL — balance + anchor, overridden by flight
    # ═══════════════════════════════════════════════════
    draw_box(XL, Y_COMP1, BOX_TEXTS['balance'], pal['wheel_fc'])
    plus(XL, Y_PLUS1)
    draw_box(XL, Y_COMP2, BOX_TEXTS['anchor'], pal['wheel_fc'])
    blend(XL, Y_PLUS2)
    draw_box(XL, Y_COMP3, BOX_TEXTS['flight'], pal['wheel_fc'])
    arrow(XL, Y_COMP3 - 0.05, Y_OUT + OUT_H + 0.08)
    draw_out(XL, Y_OUT, 'τ_w = (1−g_flt)·(τ_bal + g_anc·τ_anc) + g_flt·τ_flt')

    # ═══════════════════════════════════════════════════
    # LEG CHANNEL — posture (split height refs) + lateral/yaw
    # ═══════════════════════════════════════════════════
    draw_box(XR, Y_COMP1, BOX_TEXTS['posture'], pal['leg_fc'])
    plus(XR, Y_PLUS1)
    draw_box(XR, Y_COMP2, BOX_TEXTS['latyaw'], pal['leg_fc'])
    arrow(XR, Y_COMP2 - 0.05, Y_OUT + OUT_H + 0.08)
    draw_out(XR, Y_OUT, 'τ_q = τ_post(hᴸ, hᴿ) + τ_lat + τ_yaw')

    # ═══════════════════════════════════════════════════
    # DASHED GATE CONNECTIONS
    # ═══════════════════════════════════════════════════
    # Routed down the inner corridor between each column and the centre divider.
    # The outer margins are no longer wide enough: the boxes reach x = 0.05 / 9.95
    # and the channel headers are wider still, so an outer rail would cross both.
    BOX_L, BOX_R = XL + BW/2 + 0.2, XR - BW/2 - 0.2     # inner padded box edges
    RAIL_L, RAIL_R = BOX_L + 0.45, BOX_R - 0.45
    DASH = dict(ls='--', color='#555', linewidth=0.7, zorder=0, alpha=0.8)

    def gate_feed(rail, box_edge, y_from, y_targets):
        ax.plot([rail, rail], [y_from, min(y_targets)], **DASH)
        for y in y_targets:
            ax.plot([rail, box_edge], [y, y], **DASH)

    # prox / env / theta / flight gates → anchor and contact-loss boxes
    gate_feed(RAIL_L, BOX_L, Y_COND - 0.28,
              [Y_COMP2 + BH*0.5, Y_COMP3 + BH*0.5])
    # terrain gate → posture box (it splits the two height references), and it
    # also retunes the wheel loop across a step — see the note below.
    gate_feed(RAIL_R, BOX_R, Y_COND - 0.28, [Y_COMP1 + BH*0.5])

    # ═══════════════════════════════════════════════════
    # CENTRE DIVIDER (short, decorative)
    # ═══════════════════════════════════════════════════
    ax.plot([XC, XC], [Y_OUT - 0.2, Y_HEAD + 0.1],
            color='#ccc', linewidth=1.2, zorder=0)

    # ═══════════════════════════════════════════════════
    # BOTTOM NOTE
    # ═══════════════════════════════════════════════════
    # The gate definitions used to be spelled out in three boxes here. They repeat
    # Eqs. (10)-(13) of the manuscript verbatim, and carrying them made the figure
    # tall enough that its type shrank below 4 pt in a single column.
    Y_NOTE = Y_GATE_HDR - 0.30
    Y_MIN = Y_NOTE - 0.45
    ax.set_ylim(Y_MIN, Y_MAX)

    ax.text(XC, Y_NOTE,
            'Channels are independent on level ground; across a step g_terrain also retunes the wheel loop',
            ha='center', fontsize=5.0, color='#666', style='italic')

    # ═══════════════════════════════════════════════════
    # SELF-CHECK
    # ═══════════════════════════════════════════════════
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()

    overlaps = []
    text_items = [(c, c.get_window_extent(renderer)) for c in ax.get_children()
                  if isinstance(c, plt.Text)]
    text_items = [(t, b) for t, b in text_items if b and b.width > 5 and b.height > 5]

    for i in range(len(text_items)):
        for j in range(i+1, len(text_items)):
            ti, bi = text_items[i]; tj, bj = text_items[j]
            if not bi.overlaps(bj):
                continue
            ti_t = ti.get_text()[:40].strip() or '·'
            tj_t = tj.get_text()[:40].strip() or '·'
            if ti_t == '+' or tj_t == '+':
                continue
            if ti_t == tj_t:
                continue
            x_ov = min(bi.x1, bj.x1) - max(bi.x0, bj.x0)
            y_ov = min(bi.y1, bj.y1) - max(bi.y0, bj.y0)
            if x_ov <= 15 and y_ov <= 15:      # ignore ≤15px trivial touches
                continue
            overlaps.append((ti_t, tj_t, x_ov, y_ov))

    # channel headers must clear the dashed gate rails and each other
    inv = ax.transData.inverted()
    for h, rail in ((hdr_l, RAIL_L), (hdr_r, RAIL_R)):
        b = h.get_window_extent(renderer)
        x0, x1 = inv.transform((b.x0, 0))[0], inv.transform((b.x1, 0))[0]
        if x0 - 0.05 < rail < x1 + 0.05:
            overlaps.append((h.get_text(), 'CROSSES GATE RAIL', 0, 0))

    # text must stay inside its own definition box (wrapping can outgrow it)
    for t, rect in gate_items:
        tb = t.get_window_extent(renderer)
        rb = rect.get_window_extent(renderer)
        if tb.x0 < rb.x0 - 1 or tb.x1 > rb.x1 + 1 or tb.y0 < rb.y0 - 1 or tb.y1 > rb.y1 + 1:
            overlaps.append((t.get_text()[:40], 'OVERFLOWS ITS BOX', 0, 0))

    if overlaps:
        print(f"\n*** {name}: {len(overlaps)} OVERLAPS ***")
        for ti, tj, x, y in overlaps[:15]:
            print(f"  '{ti}' <> '{tj}' (x={x:.0f} y={y:.0f}px)")
        print("FAILED")
    else:
        print(f"{name}: zero overlaps — PASSED")

    for ext in ('.pdf', '.png'):
        plt.savefig(OUT_DIR + name + ext, dpi=300, bbox_inches='tight', pad_inches=0.06)
    plt.close(fig)
    print("Fig.2 saved to", OUT_DIR + name, "(.pdf + .png)")


for _name, _pal in PALETTES.items():
    build(_name, _pal)
