"""Compute equilibrium keyframe using inverse kinematics with root Z adjustment.

Strategy:
1. Target: CoM at height 0.4m, aligned with wheel center (Y=0)
2. Optimize over hip_pitch, knee, AND root_z to find stable configuration
...
def compute_com_and_contacts(root_z, hip_pitch, knee):
    """Compute CoM position and contact forces for given configuration."""
    # Reset to keyframe
    ...
    # Set configuration
    mj_data.qpos[2] = root_z        # root Z position
...
def objective(x):
    ...
    root_z, hip_pitch, knee = x

    try:
        com_pos, wheel_center_y, total_fz, total_mass = compute_com_and_contacts(root_z, hip_pitch, knee)
        ...
        # Penalize extreme joint angles (prefer natural standing posture)
        # Hip pitch around 30-45 degrees, knee around 90-100 degrees
        hip_pitch_natural = 0.6  # ~34 degrees
        knee_natural = 1.6       # ~92 degrees
        posture_error = 0.1 * ((hip_pitch - hip_pitch_natural) ** 2 + (knee - knee_natural) ** 2)

        # Combined objective
        cost = 1000 * height_error + 1000 * y_offset_error + 10 * force_error + posture_error
...
x0 = [0.545, 0.674267, 1.668071]  # root_z, hip_pitch, knee

# Bounds: reasonable ranges
bounds = [
    (0.40, 0.70),   # root_z: 40cm to 70cm
    (0.3, 1.2),     # hip_pitch: 17 to 69 degrees
    (1.0, 2.0),     # knee: 57 to 115 degrees
]
...
if result.success:
    root_z_opt, hip_pitch_opt, knee_opt = result.x
    print()
    print(f'Optimization successful!')
    print(f'  root_z: {root_z_opt:.6f} m')
    print(f'  hip_pitch: {hip_pitch_opt:.6f} rad ({np.degrees(hip_pitch_opt):.2f} deg)')
    print(f'  knee: {knee_opt:.6f} rad ({np.degrees(knee_opt):.2f} deg)')
    print()

    # Verify solution
    com_pos, wheel_center_y, total_fz, total_mass = compute_com_and_contacts(root_z_opt, hip_pitch_opt, knee_opt)
