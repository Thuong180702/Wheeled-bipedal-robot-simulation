...
measured under the single protocol of Sec.~\ref{sec:standing}; values are
...
twenty-nine minutes at a \SI{-26.763}{mm} parking offset, the only motion left
a \SI{0.14}{mm/hour} lateral creep on the passively held axis. The separation
from the full-state LQR of
...
$R_{\text{sag}}$ to \SI{3.97}{mm} (M3), the gated boost the rest of the
way---a combined factor of \num{86}. \emph{(ii)
...
\SIrange{82.1}{144.1}{N}: the two lateral
bearings average \SI{98.0}{N} against \SI{130.0}{N} for the two sagittal ones,
and the weakest bearing of all is the oblique \SI{45}{\degree} (\SI{82.1}{N},
...
and the proximity gate add nothing to push \emph{margin} on their own---the gate
prevents windup, it does not enlarge the capture basin---and the full anchor is
...
a whole and not by any one of its four gates---including gain scheduling (S0) and
the damping boost, which dominates idle precision yet leaves push margin
indistinguishable from ACC's (S2). Only the pitch gate is isolated by the push
data: removing it (S5) leaves the robot unable to survive even the search floor.
Ringdown decays within \SI{9}{s} under ACC, whereas P-only oscillates
indefinitely on the \SI{1.3}{Nm} equilibrium bias; symmetric EMA shows the
bistable failure the asymmetry prevents, and the ungated integral winds up during
the push and never settles.
...
\num{32}$\times$. The mechanism is saturation---the roll channel is commanded
against the $\pm\SI{0.7}{rad}$ hip-roll limit throughout every episode. The
...
reduced-order designs and still does not close the problem. ACC, held to the same lengthened horizon, returns the opposite
verdict---\num{10}/\num{10} at \SI{300}{s}, \num{4}/\num{4} at \SI{1800}{s},
...
weakest axis, the \SI{98.0}{N} mean lateral bearing of Sec.~\ref{sec:push}, but
...
\begin{table}[htbp]
\centering
\caption{Robustness to sensor noise and actuator delay, $N{=}20$ per cell. Falls: trials terminated by a fall, out of $N$. Idle RMS: lateral (world $x$) CoM std.\ over \SI{20}{s} after a \SI{3}{s} settle, \emph{surviving trials only}. $F_{\max}$: binary-search lateral (world $+x$) push, $\pm\SI{5}{N}$ tolerance, floor \SI{10}{N}. Delay is quantised to the control period here and resolved five times more finely in the text.}
...
optimistic and its falls column is the primary metric. Produced by
...
budget of \S\ref{sec:discussion} lands inside that bracket rather than clear of
it. Losing the push margin and losing the platform are different thresholds: the
standing threshold is \SIrange{14}{16}{ms}, station held through \SI{14}{ms} at
\SI{10.84}{mm} idle lateral RMS.
The knee is not an exhausted margin: at \SI{8}{ms} each of four profile
constants sits in a sharp \emph{local dip} at its shipped value (\SI{51.0}{N},
nearly every neighbouring value recovering \SIrange{78}{93}{N}), and doubling
the sagittal velocity damping to \SI{45}{Nm/(m/s)}---ACC-DH---moves the knee
outward to \SIrange{8}{10}{ms} ($51.0\rightarrow\SI{93.2}{N}$ at \SI{8}{ms}) for
$+\SI{1.4}{\percent}$ idle lateral RMS. It is reported as an evaluation variant
and not adopted, that constant also governing driving acceleration and
settling.
...
impulse followed \SI{2}{s} later, before the first ringdown has decayed, by a
\SI{60}{N} counter-directed one is survived \textbf{10/10} at peak pitch
$11.89{\pm}\SI{0.51}{\degree}$---what
...
decades (\numrange{e-4}{e-2}) decay in the full nonlinear simulator---adaptive
bias live, nothing frozen---at $\rho{=}0.9951$--$0.9955$ against the
...
Welch's $t$-test at Bonferroni-adjusted $\alpha{=}0.0056$ (9 comparisons) makes
L0$\rightarrow$L1 and L2.5$\rightarrow$L3 significant ($p{<}\num{1e-15}$; Cohen's
$d{=}4.3$ and $3.6$) and bounds the four subtractive removals rather than merely
failing to reject them ($|d|{\le}0.49$, ${<}\SI{1.4}{N}$); $N{=}10$ success rates
carry a Clopper--Pearson 95\% CI of $[0.69,1.00]$, the $N{=}30$ push ablations
resolve ${\sim}\SI{1}{N}$, and no claim rests on less.
...
is built for---residual learning \emph{over} the prior---is not run. Predictive control is the remaining untested
...
action cannot reject, and offset-free MPC rejects it by augmenting a disturbance
state---i.e.\ by adding integral action, which is what the anchor is. Preview
would buy something in the push regime, where the torque and
$\pm\SI{0.7}{rad}$ hip-roll bounds do bind, and across the height band through
relinearisation: the two comparisons left open, against a dense QP at
\SI{100}{Hz} versus a branchless \num{27664}-flop step.
...
control step (p99 over five repeats of \num{5000} steps). Host wall-clock is
nevertheless the wrong quantity to extrapolate to an embedded target; the
reproducible one is the operation count: \num{27664} floating-point operations
and \num{159} transcendentals per step over \num{1001} scalars, identical on
every repeat, with no iterative solve, no matrix factorization and no
data-dependent control flow---every gate of \S\ref{sec:anchor} is a branchless
select---so worst-case execution time equals average-case, which a QP-based
controller cannot offer. A \SI{480}{MHz} Cortex-M7 retires that in
${\sim}\SI{0.07}{ms}$, so compute is the one budget line that shrinks on the
target: ${\sim}\SI{95}{\percent}$ of the measured \SI{1.2}{ms} is CoM and
support-polygon work plus Python dispatch that a C port deletes. Charged at
\SI{0.1}{ms} the estimate is \SI{6.6}{ms}, so readout and actuation decide the
matter, not compute. Two mitigations compose---a real-time OS recovers
\SIrange{2}{4}{ms}, and doubling the sagittal velocity damping moves the knee to
...
from $F_{\min}{=}\SI{82}{N}$ to a \SI{108}{N} median; absolute accuracy is
separate---the robot parks \SI{27.0}{mm} from commanded home and holds that
...
whose gating encodes the plant's measured failure modes. The residual stack
it was built to serve is future work, alongside hardware validation, a resourced
learning comparison, and conditional activation for locomotion.
...
table's note names its script. The extended version there
carries the full derivations, WBC details and parking-offset model.

\num{31} supplementary clips are under \path{paper/videos/}, rendered by
\path{scripts/render_paper_videos.py} from the same harness and seeds that
produced the corresponding table, so no clip can drift from the number it
illustrates: quiet stance (1); push recovery (10---eight bearings at \SI{75}{N},
a \SI{90}{N} lateral recovery, and a deliberate \SI{85}{N} oblique \emph{fall}
bracketing the weakest bearing of Table~\ref{tab:push_ablation} from above);
drops \SIrange{10}{100}{cm} (6); one-wheel curbs (3); ramp and ledge
\SIrange{20}{50}{cm} (8); height transitions (3). Each is one trial at one seed,
illustrating a regime rather than establishing a rate; that directory's
\path{README.md} gives the per-clip protocol and the outcome each one backs. The
robustness sweep, baseline comparison and ablation are not filmed---all
distinguish their arms by numbers no viewer can see on a robot standing
still.
