...
\midrule
\emph{Ablations} \\
\quad symmetric EMA         & 0.73 & never\textsuperscript{1} & ${<}5\%$ \\
\quad global I              & fall & ---\textsuperscript{2} & --- \\
\quad no boost               & 0.73 & never decays & ${<}5\%$ \\
\bottomrule
\end{tabular}
\vspace{2pt}\par\noindent
...
\textsuperscript{3}Idle precision matches ACC (\SI{0.73}{mm}); the proximity gate alone determines idle precision---neither the damping boost nor the asymmetric envelope affects steady-state standing (cf.\ S2, S3 in Table~\ref{tab:push_ablation}).
