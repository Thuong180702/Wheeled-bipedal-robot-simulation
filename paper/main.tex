...
Preliminary PPO~\cite{schulman2017ppo} training (5.4M steps, single seed, in our \texttt{BalanceEnv}) showed limited height generalization: the policy stood at a single height (Surv ${\approx}\SI{3.20}{s}$, vs.\ \SIrange{0.16}{0.60}{s} for the classical baselines) but failed to generalize across \SIrange{0.40}{0.70}{m} base-$z$ (\S\ref{sec:height_convention})---height RMSE ${>}\SI{50}{mm}$, \SI{85}{\percent} falls on squat transitions---with the failures concentrated on precisely the coupled pitch--height dynamics that a posture map and gate schedule encode explicitly. This run is retained throughout as a \emph{difficulty probe}, not as a reinforcement-learning baseline. Its purpose is to characterize how hard unaided model-free search is on the coupled pitch--height state space of a 10-DOF wheeled biped, and thereby to make explicit what a structured prior supplies in its place; it is single-seed, without domain randomization or a hyperparameter search, and no claim about the attainable performance of RL on this platform is drawn from it.
...
\begin{enumerate}
    ...
    \item A \textbf{factorial ablation} (additive L0$\rightarrow$L3 $+$ subtractive S0$\rightarrow$S5) with six classical baselines (four 4-state-derived and two 6-state coupled) and a preliminary model-free PPO reference (5.4M steps, single seed) included as a difficulty probe.
\end{enumerate}
...
Signal-to-noise ratios against a simulator noise floor are omitted here. The
floor is obtained by locking the joints and pinning the base
(\path{run_r2_between_trial_stats.py --noise-floor}); because that procedure
zeroes velocities without welding the base, it is trustworthy on the
laterally-constrained axis (\SI{0.0019}{mm} RMS) but introduces artificial sag
on the gravity-loaded sagittal and vertical axes, where the residuals it
reports (\SI{5.79}{mm} and \SI{63.5}{mm}) reflect the pinning rather than
integrator noise. Precision is therefore assessed from the relative spread
...
\SI{27.0}{mm}. An ideal integrator would drive it to zero. We report the discrepancy separately rather than absorbing it into a planar
average where it is less visible, but it is not an unexplained residual.
Appendix~\ref{sec:parking_supp} instruments the sagittal torque balance and
derives the offset in closed form as the static consequence of two settings: a
\emph{leaky} anchor integral, whose ${\sim}\SI{2}{s}$ forgetting horizon fixes
its DC gain at $k_{I,\text{dc}}{=}\SI{7.96}{Nm/m}$, and a \SI{1.44}{\degree}
error in the feedforward pitch reference. The prediction is \SI{24.88}{mm}
against \SI{24.99}{mm} measured, and it tracks a $40{\times}$ gain sweep and a
five-point height sweep to within \SI{0.92}{\percent}. The leak is not free to
remove---a $10{\times}$ smaller $\lambda$ cuts the offset to \SI{-16.31}{mm}
but degrades window jitter twelvefold ($0.298\rightarrow\SI{3.463}{mm}$)---so
what Table~\ref{tab:standing} publishes is a priced design point rather than a
defect (Limitation~8).
...
alone suffices. The distinction is worth drawing because the mechanism was
introduced to prevent a nose-down crash observed in an earlier controller
...
\item \textbf{PPO reference (preliminary)}---Model-free policy trained from scratch for \num{5.4}M environment steps in \texttt{BalanceEnv}: single seed, no domain randomization, no hyperparameter search. Included as a difficulty probe, not as a tuned RL baseline (Limitation~10).
...
\textbf{Outcome.} Fall rate is \SI{100}{\percent} in all twelve baseline cells. The \SI{100}{Hz} columns are the ones carried into Table~\ref{tab:lqr_baselines}; the \SI{50}{Hz} columns come from the same script and the same episodes, so the two rates differ only by the intended change. ACC is shown at \SI{100}{Hz} only; the \SI{50}{Hz} ACC run is analysed as a negative result in Limitation~5 and is not a usable row.
...
The converse experiment---running ACC at \SI{50}{Hz}---is a negative result: the controller does not survive the rate change, and it is recorded as a limitation (\S\ref{sec:discussion}, Limitation~5) rather than as a rate-independence claim. What Table~\ref{tab:rate_matched} establishes is the direction that matters for the comparison: the classical baselines fail both at the rate for which they were designed and at ACC's rate.
...
A factorial sweep over four noise levels and a delay axis ($N{=}20$ per cell, \num{240} trials) was extended along the clean-delay axis by a separate $N{=}5$ sweep. An audit of the harness (\path{scripts/collect_robustness_sweep.py}) found that its delay line holds a \emph{single} slot overwritten every control step, so any non-zero delay request applies exactly one control period of transport lag irrespective of the value requested; the delay argument sets only a negligible warm-up window. The cells reported in earlier drafts as \SI{30}{ms}, \SI{50}{ms}, \SI{100}{ms} and \SI{150}{ms} are consequently all \SI{10}{ms} cells, and the extended sweep inherits the defect because it invokes the same measurement script. Table~\ref{tab:robustness} therefore states a two-level delay axis (\SI{0}{ms} and \SI{10}{ms}) and retains the affected cells as what they in fact are---independent seed-samples of one \SI{10}{ms} configuration. The consequence for the claims is specific and we state it plainly: \emph{no delay above \SI{10}{ms} has been tested on this platform}, so no stability threshold is claimed, and the delay axis proper is resolved instead by the dedicated sweep of Table~\ref{tab:delay_cliff}. What the repeats do establish is that the post-bifurcation state is reproducible rather than marginal: over \num{50} clean trials in five independent samples, $F_{\max}$ lands in \SIrange{45.3}{49.4}{N} and idle RMS in \SIrange{2.23}{2.61}{mm}, with zero falls in all fifty. Against the \SI{0}{ms} reference (\SI{95.7}{N}, \SI{0.43}{mm}) that is a bifurcation of $F_{\max}$ to roughly half and a $5.6{\times}$ rise in idle RMS.
Delay at \SI{10}{ms} is therefore benign for stability and costly for performance---consistent with ACC's gates depending on physical state (position, velocity envelope, orientation) rather than on tight timing---but whether that remains true at larger delays is untested rather than established.
Noise alone is likewise survivable up to the medium level (\num{0} falls in \num{20} at \SI{0}{ms} delay).
What ACC does not tolerate is the \emph{conjunction}: medium noise together with \SI{10}{ms} of delay costs \num{7} falls in \num{40} trials, the only sub-catastrophic falls anywhere in the grid. High noise is unrecoverable at every delay (\num{60}/\num{60}); the asymmetric envelope cannot distinguish noise from motion, forcing anchor disengagement, and the residual P-only mode cannot hold the platform.
...
Idle repeatability degrades from \SI{0.43}{mm} (clean) to \SI{30.82}{mm} under medium sensor noise at zero delay---a \num{71}-fold loss. The comparison that makes this concrete is against the un-anchored baseline on the same axis: L0 holds $R_{\text{lat}}=\SI{3.675}{mm}$ (Table~\ref{tab:standing}), so under medium sensor noise ACC is roughly an order of magnitude \emph{worse} than having no anchor at all. The mechanism is the one described above---the envelope gate cannot separate sensor noise from real motion, so it releases the integral and then re-engages it repeatedly---and the practical consequence is that ACC as presented requires a filtered velocity estimate, not a raw one. Three caveats attach to the absolute values in this table. First, its idle column is measured on the \emph{lateral} axis (world $x$), not the sagittal one of Table~\ref{tab:standing}, so the two are not directly comparable in level; the like-for-like clean reference here is ACC's lateral $R_{\text{lat}}=\SI{0.730}{mm}$, and the \SI{0.43}{mm} entry is lower still because of the shorter settling allowance (\SI{3}{s} here vs.\ \SI{5}{s} there): with EMA$_0{=}0$ the envelope gate opens fully at the start of the trial, so a window that begins earlier captures more of the strongly-anchored transient and less of the steady state. Second, that column averages surviving trials only, so in the two cells that produce falls it is mildly optimistic---a configuration that falls more can report a \emph{better} idle number, and the falls column must be read first. Third, the noise and delay \emph{ratios} are unaffected by any of these issues, since every cell in the table is measured identically; it is the degradation factors, not the absolute millimeters, that this table is used for. Re-running the sweep with sagittal resolution and EMA$_0{=}1.0$ initialization would remove the first caveat and is deferred.

\begin{table}[t]
\centering
\caption{Robustness to sensor noise and actuator delay. Falls: trials terminated by a fall, out of $N$. Idle RMS: lateral (world $x$) CoM std.\ over \SI{20}{s} after \SI{3}{s} settle (EMA$_0{=}0$ transient), \emph{surviving trials only}. $F_{\max}$: binary-search lateral (world $+x$) push, $\pm\SI{5}{N}$ tolerance, search floor \SI{10}{N}. Noise$\times$delay grid $N{=}20$; extended rows $N{=}5$. Lettered rows (a)--(e) are \emph{independent seed-samples of the same \SI{10}{ms} configuration}, not distinct delays---see note $\ddagger$. The delay axis proper is resolved in Table~\ref{tab:delay_cliff}.}
...
Clean,     \SI{10}{ms} delay (a) & 0/20 & 2.40 & 0.25 & 45.5 & 3.1 \\
Clean,     \SI{10}{ms} delay (b) & 0/20 & 2.51 & 0.24 & 49.4 & 6.7 \\
Clean,     \SI{10}{ms} delay (c)$^\dagger$ & 0/5 & 2.61 & 0.42 & 49.1 & 7.6 \\
Clean,     \SI{10}{ms} delay (d)$^\dagger$ & 0/5 & 2.23 & 0.25 & 48.3 & 3.8 \\
Clean,     \SI{10}{ms} delay (e)$^\dagger$ & 0/5 & 2.40 & 0.22 & 45.3 & 1.5 \\
\midrule
Low noise, \SI{0}{ms} delay  & 0/20 & 5.86 & 2.53 & 96.0 & 1.0 \\
Low noise, \SI{10}{ms} delay (a) & 0/20 & 10.88 & 3.61 & 49.9 & 9.1 \\
Low noise, \SI{10}{ms} delay (b) & 0/20 & 7.46 & 4.10 & 51.0 & 3.5 \\
\midrule
Med noise, \SI{0}{ms} delay  & 0/20 & 30.82 & 12.13 & 93.5 & 5.2 \\
Med noise, \SI{10}{ms} delay (a) & \textbf{4/20} & 31.44 & 14.08 & 45.1 & 15.3 \\
Med noise, \SI{10}{ms} delay (b) & \textbf{3/20} & 31.81 & 15.46 & 46.5 & 16.3 \\
\midrule
High noise, \SI{0}{ms} delay & 20/20 & \multicolumn{2}{c}{--- (all fell)} & \multicolumn{2}{c@{}}{---} \\
High noise, \SI{10}{ms} delay (a) & 20/20 & \multicolumn{2}{c}{--- (all fell)} & \multicolumn{2}{c@{}}{---} \\
High noise, \SI{10}{ms} delay (b) & 20/20 & \multicolumn{2}{c}{--- (all fell)} & \multicolumn{2}{c@{}}{---} \\
\bottomrule
\end{tabular}
\vspace{2pt}\tabnote\noindent
$^\dagger$Rows from the separate extended sweep, $N{=}5$; all other rows $N{=}20$. Noise (1-$\sigma$): low = \SI{0.01}{rad/s} (ang.\ vel.), \SI{0.01}{m/s^2} (grav.), \SI{0.001}{rad} (joint pos.); med = $5\times$ low; high = $10\times$ low.
$^\ddagger$\emph{Harness correction.} These cells were requested at \SI{0}{ms}, \SI{10}{ms} and \SI{30}{ms} (and, for the extended rows, \SI{50}{ms}, \SI{100}{ms} and \SI{150}{ms}), but the harness stores the delayed command in one slot overwritten every control step, so every non-zero request applied exactly one control period (\SI{10}{ms}) of lag. The rows are retained unaltered and relabelled: they are five clean and two per-noise-level independent samples at \SI{10}{ms}, and the spread among them is seed variation within one configuration. No delay above \SI{10}{ms} is measured anywhere in this table, and none is claimed.
Performance bifurcation at \SI{10}{ms} ($F_{\max}$: \SI{95.7}{N}$\rightarrow{\sim}\SI{46}{N}$, idle \SI{0.43}{mm}${\rightarrow}{\approx}\,$\SI{2.5}{mm}) is reproducible across all five clean samples with zero falls in \num{50} trials.
High noise is unrecoverable at both delays (\num{60} of \num{60} trials): it holds the velocity envelope above threshold, disengaging the anchor, and the residual P-only mode cannot hold the platform under that noise level. The low-noise idle spread (\SI{10.88}{mm} in sample (a) vs.\ \SI{7.46}{mm} in sample (b)) is between two samples of the same configuration and is therefore seed variation; earlier drafts read it as a delay-dependent resonance, which the harness correction rules out.
Because the idle column averages survivors only, the medium-noise rows are mildly optimistic where falls occur; the falls column is the primary metric for those cells.
\tabnote\noindent
...
\textbf{Limitations.}
(1)~The push envelope is anisotropic, and the anisotropy is structural rather than a deficiency to be tuned out: the two lateral bearings average \SI{97.5}{N} against \SI{131.8}{N} for the two sagittal ones because the wheels can drive the contact patch back under the CoM sagittally but produce no lateral ground force at all, and the weakest bearing is the oblique \SI{45}{\degree} ($\SI{82.8}{N}$, \SI{29}{\percent} below $F_{\text{med}}$), where neither mechanism has full authority. Raising the lateral bearing requires a centralized lateral task (\S\ref{sec:wbc}), not a gain change. What is genuinely unresolved is the \emph{chirality}: one oblique bearing survives \SI{40}{N} more than its mirror (Fig.~\ref{fig:push_polar}), reproducibly across trials, and we have not traced it to a mechanism.
(2)~\textbf{Oblique ledge descent fails in a straddle regime rather than above a step-height threshold, and the binding constraint is the per-leg span of \S\ref{sec:terrain}, not lateral balance authority.} An oblique exit releases the two wheels a stagger interval $w_{\text{track}}\tan\phi / v$ apart, and only within that interval can the leg split absorb the step. Instrumenting a \SI{30}{\degree}, \SI{20}{cm} exit at \SI{0.7}{m/s} shows the window closing in \SI{0.18}{s}: the split had commanded \SI{25}{mm} of differential leg travel against the \SI{200}{mm} step when the trailing wheel lost contact, and the torso subsequently absorbed the remainder, reaching $\SI{-33.1}{\degree}$ of roll against the $\arctan(0.20/w_{\text{track}}) = \SI{35.7}{\degree}$ that a wholly unspanned step predicts. The legs were not at their travel limit when contact was lost (\SI{99}{mm} of the calibrated range unused), so the proximate failure is a race rather than a saturation---but relieving it would not rescue the case, because the full \SI{20}{cm} is itself unspannable at nominal stance (\SI{170}{\degree} of knee flexion against the \SI{2.7}{rad} limit, \S\ref{sec:terrain}). The \SI{\pm 6}{cm} leveling trim is unavailable throughout, since its gate requires \emph{both} wheels loaded. Failure is consequently \emph{not} monotone in step height: on a deterministic course at one trial per cell, a \SI{45}{\degree} exit settles at \SI{30}{cm} and a \SI{30}{\degree} exit at \SI{40}{cm}, while both angles fail at \SI{20}{cm}. Two regimes separate them, set by whether the leading wheel reaches the lower ground before the trailing wheel releases---free fall over the step takes $\sqrt{2h/g}$, comparable to the stagger interval at the heights tested. A shallow step lands the leading wheel while the trailing wheel still bears on the platform and forces the straddle the legs cannot span; a deep step releases both wheels together and is handled by the flight path of \S\ref{sec:flight} (Table~\ref{tab:flight_ext}). We do not claim a calibrated boundary between the two regimes: the mapping was measured at one trial per cell over four heights and two angles, which fixes the mechanism but not the threshold. Widening the straddle regime requires knee range rather than gains; the alternative---detecting an unspannable step and committing deliberately to the launch regime---has not been evaluated.
(3)~All results are simulation-only (MuJoCo); hardware validation remains future work.
(4)~\textbf{The push-margin cliff sits below the estimated latency budget, and the delay tolerance above \SI{10}{ms} is unmeasured.} Push margin collapses across a knee between \SI{6}{ms} and \SI{8}{ms} of actuator delay: $F_{\max}$ holds within \SI{2.4}{\percent} of its \SI{0}{ms} value out to \SI{6}{ms} and then falls by \SI{45}{\percent} (Table~\ref{tab:delay_cliff}), with idle RMS rising $5.5{\times}$ across the same transition. The end-to-end budget estimated below is \SI{9.5}{ms}, so the design as specified sits \SIrange{2}{3.5}{ms} \emph{past} the cliff rather than short of it, and the real-time measures discussed there become the condition for clearing it rather than a comfort margin. Because the transition is a cliff and not a roll-off, that margin cannot be part-recovered: a budget of \SI{7.5}{ms} performs like \SI{10}{ms}, and one of \SI{5.5}{ms} like \SI{0}{ms}. Two further gaps are open. First, the substep sweep that locates the knee is clean-sensing only; the interaction with sensor noise---the conjunction that produces the only sub-catastrophic falls in Table~\ref{tab:robustness}---has been measured at \SI{10}{ms} but not across the knee. Second, \emph{no delay above \SI{10}{ms} has been tested}: an earlier draft reported zero falls out to \SI{150}{ms}, but the harness that produced those rows applies one control period of lag regardless of the delay requested (\S\ref{sec:analysis}), so we withdraw that claim entirely and assert no stability threshold. That ACC's gates key on physical state rather than on tight timing remains a structural argument for delay tolerance, not a measurement. What can be said in mitigation is that the knee belongs to the tuning and not to the architecture: a one-knob screen at \SI{8}{ms} finds the shipped gain set sitting in a sharp local dip, and doubling the sagittal velocity damping moves the knee to \SIrange{8}{10}{ms}---clear of the \SI{9.5}{ms} budget---at no measured cost to quiet stance and with improved margin under noise (Tables~\ref{tab:delay_cliff},~\ref{tab:delay_screen}). That variant is reported, not adopted, because the constant it changes also governs driving and settling and has not been re-qualified over the \num{48}-scenario suite; so the mitigation is demonstrated but not yet paid for. Measuring where the robot actually falls, and re-qualifying a delay-hardened default, are the two pieces of work this limitation names.
(5)~\textbf{ACC is not rate-portable downward.} A \SI{50}{Hz} ACC evaluation with recomputed discrete coefficients ($\alpha_a{\approx}0.58$, $\alpha_r{\approx}0.013$, anchor leak~\num{0.010}) \emph{failed}---the robot collapsed to a mean CoM height of \SI{0.124}{m} (nominal \SI{0.404}{m}), and the sub-millimeter idle RMS and \SI{10}{N} push threshold recorded by that run are measurements taken on a fallen robot. Rate-independence of ACC's architectural contribution is therefore not claimed, and the responsible mechanism remains undetermined between two candidates: excitation of high-frequency plant modes that the \SI{100}{Hz} loop suppresses, or the phase lag introduced by discretizing the asymmetric envelope at $\Delta t{=}\SI{20}{ms}$, where $\alpha_a{\approx}0.58$ leaves the attack filter barely more than one sample from deadbeat. Distinguishing them requires a loop-gain and phase-margin analysis of the linearized closed loop at both rates, together with a re-tuning of the ACC gains for the \SI{20}{ms} step; neither has been performed. The practical consequence is a deployment constraint rather than a comparison flaw: ACC as tuned requires a \SI{100}{Hz} loop, and the \SI{9.5}{ms} end-to-end budget estimated below leaves no room to relax it.
...
(7)~The classical comparison is bounded by what could be linearized. A full-state LQR from finite-difference linearization of the complete 10-DOF MuJoCo plant was attempted but produced unreliable gain matrices due to ill-conditioned contact dynamics at the standing equilibrium ($\kappa(A){\approx}10^{10}$), so all six baselines of \S\ref{sec:baselines} derive from reduced 4- and 6-state models; robust linearization of articulated-leg contact dynamics at unstable equilibria is an open challenge. The hip-roll coupling $b_{r,h}{=}-5.0$ in the 6-state LQR is empirical; a sensitivity sweep is deferred.
(8)~\textbf{Absolute sagittal accuracy is capped at ${\sim}\SI{27}{mm}$ by a deliberate design setting with a measured price, not by an unexplained residual.} ACC's position integral acts on the sagittal axis, yet the robot settles $\SI{-27.0}{mm}$ from the commanded home along that axis and then holds the offset to within \SI{0.009}{mm} SD across ten trials (Table~\ref{tab:standing}). Sagittal \emph{repeatability} is excellent (\SI{0.294}{mm}), so the deficit is a static bias rather than motion, and it is largely insensitive to the gate structure (every subtractive ablation that survives lands within \SI{0.2}{mm} of it). Direct instrumentation of the sagittal torque decomposition (Appendix~\ref{sec:parking_supp}) accounts for it completely. At the parking point the gravity moment about the wheel axle is $\SI{0.0040}{Nm}$ (CoM lever \SI{0.050}{mm})---a settled wheeled inverted pendulum necessarily parks its mass over the contact line---and the anchor integral sits at \SI{0.198}{Nm}, \SI{10.0}{\percent} of its \SI{2.0}{Nm} clamp, so the offset is neither gravity-driven nor a windup or saturation artifact. What actually balances is a constant pitch-loop demand of $\SI{-1.259}{Nm}$, produced because the feedforward pitch reference commands $+\SI{3.476}{\degree}$ where the robot in fact balances at $+\SI{2.034}{\degree}$. The position loop must cancel that torque, and because the anchor integral is \emph{leaky}---deliberately, so that it forgets a disturbance in ${\sim}\SI{2}{s}$ rather than accumulating one indefinitely---its DC gain is finite ($k_{I,\text{dc}}{=}k_i\Delta t(1{-}\lambda)/\lambda{=}\SI{7.96}{Nm/m}$, one fifth of $k_{pos}$), so a nonzero error is required to produce it: Eq.~\eqref{eq:parking} predicts \SI{24.88}{mm} against \SI{24.99}{mm} measured, and tracks a $40{\times}$ gain sweep and a five-point height sweep to within \SI{0.92}{\percent}. Only the residual \SIrange{1}{3}{mm} is a frame-convention gap (\S\ref{sec:height_convention}). The leak is the term that cannot simply be dialled away: a $10{\times}$ smaller $\lambda$ does cut the offset to \SI{-16.31}{mm}, but window jitter degrades twelvefold ($0.298\rightarrow\SI{3.463}{mm}$) and the forgetting horizon stretches to \SI{20}{s}---precisely the trade the shipped setting was chosen to avoid. The two cheaper routes were measured instead, and both carry a price. Raising $k_i$ from \num{4} to \num{160} removes \SI{79.5}{\percent} of the offset and costs nothing on \emph{push} ($p{\geq}0.32$ on both $F_{\min}$ and $F_{\text{med}}$), but a five-point gain sweep re-measured across the whole evaluation suite locates the cost elsewhere: in the noise-plus-delay cells of Table~\ref{tab:robustness}, the fall rate rises monotonically in trend with $k_i$ (\num{7}, \num{13}, \num{10}, \num{14}, \num{17} falls in \num{40} at $k_i{=}\num{4},\num{20},\num{40},\num{80},\num{160}$; Cochran--Armitage trend on $\log_2 k_i$, $z{=}2.34$, $p{=}0.019$; $k_i{=}160$ alone $p{=}0.027$ against the shipped setting). The cost is graded rather than a threshold, so no intermediate gain is safe: $k_i{=}40$ appears clean ($p{=}0.59$) only because \num{40} trials cannot resolve \SI{17.5}{\percent} from \SI{25}{\percent}. Recalibrating the feedforward pitch reference removes \SI{92.5}{\percent} ($-27.0\rightarrow\SI{-2.0}{mm}$) with $F_{\min}$ unchanged (\SI{82.5}{N} vs.\ \SI{82.8}{N}, $p{=}0.55$), at the cost of a borderline \SI{4.5}{N} reduction in $F_{\text{med}}$ ($p{=}0.051$) and a per-height rather than scalar correction (\SIrange{0.94}{1.97}{\degree} over \SI{2}{cm}). Neither is promoted into the shipped profile. The \SI{27}{mm} is therefore not an unpaid debt but a purchased margin: it buys a leak short enough to forget a disturbance in \SI{2}{s} and an integral gain low enough that medium sensor noise combined with actuator delay does not tip the platform. What remains genuinely open is narrow: whether the \SI{4.5}{N} pitch-recalibration cost survives a larger sample, whether a lead-compensated or bandwidth-limited integral could raise DC gain without raising mid-band gain (\S\ref{sec:parking_supp}), and the \SIrange{1}{3}{mm} convention gap.
(9)~Architectural generality across morphologies remains to be demonstrated.
(10)~Push ablation at $N{=}10$ has min.\ detectable effect ${\sim}\SI{3.2}{N}$; sub-\SI{2}{N} differences non-significant (see Statistical power). The model-free PPO run (5.4M steps, single seed, no domain randomization, no hyperparameter search) does not meet the standard of a tuned reinforcement-learning baseline and is not presented as one; it is a difficulty probe on the unaided search problem, and the \SI{85}{\percent} fall rate is reported as observed rather than as an upper bound on RL performance for this platform. A properly resourced learning comparison---multiple seeds, domain randomization~\cite{tobin2017domainrand,peng2018dynrand}, a tuned reward---lies outside the scope of a structured-prior contribution and remains open; the natural form of that comparison is residual learning over the prior rather than learning against it. A task-priority QP whole-body controller was benchmarked both standalone and as an assist on ACC and did not resolve the precision--recovery trade-off (\S\ref{sec:wbc}). One caveat remains open there---roll RMS is the single axis on which the WBC blend beats ACC (\SI{14}{\percent} over the full batch, \SI{18}{\percent} on the matched subset), so the centralized formulation is not without merit on that axis; what the evidence establishes is that its advantage cannot be harvested by torque-level blending. A tuned convex MPC on the full 10-DOF plant remains deferred because finite-difference linearization at the standing equilibrium is ill-conditioned ($\kappa(A){\approx}10^{10}$, Limitation~7), so robust full-plant MPC is an open prerequisite rather than a missing comparison.

\textbf{Deployment Risks and Mitigations.}
ACC uses raw torque, \SI{400}{Nm/s} rate limit, standard sensors (IMU $+$ encoders). The binding latency figure is a \textbf{performance cliff between \SI{6}{ms} and \SI{8}{ms}}, across which $F_{\max}$ falls \SI{95.5}{N}${\rightarrow}\SI{51}{N}$ (Table~\ref{tab:delay_cliff}); below \SI{6}{ms} the cost is under \SI{2.4}{\percent}. We do \emph{not} state a companion stability threshold. Zero falls were observed in \num{50} clean trials at \SI{10}{ms}, but no larger delay has been measured (\S\ref{sec:analysis}), so the delay at which the robot actually falls is unknown. The architectural expectation is that it is far above the performance cliff, because ACC's gates depend on physical state rather than timing precision---the proximity gate is spatial (\SI{15}{cm}), the asymmetric envelope temporal but slow ($\tau_r{\approx}\SI{1.5}{s}$), and the damping boost state-driven rather than schedule-driven---but that expectation is untested and must not be commissioned against.

Estimated non-RTOS worst-case budget: sensor readout ${\sim}\SI{1}{ms}$ $+$ state estimation ${\sim}\SI{2}{ms}$ $+$ ACC computation ${\sim}\SI{3}{ms}$ $+$ actuation latency ${\sim}\SI{3.5}{ms}$ $=$ ${\approx}\SI{9.5}{ms}$ end-to-end---a budget assembled from assumed component figures, not a measurement. Read against a cliff at \SIrange{6}{8}{ms}, that budget is \emph{over} the threshold by \SIrange{2}{3.5}{ms}, so the non-RTOS configuration should be expected to deliver the post-cliff push envelope (${\sim}\SI{51}{N}$), not the nominal one. On a real-time OS with a dedicated balance-loop microcontroller (e.g., \SI{1}{kHz} RT Linux or bare-metal MCU), jitter is typically ${<}\SI{0.1}{ms}$ and \SIrange{2}{4}{ms} of budget is recoverable---which straddles the cliff rather than clearing it comfortably: \SI{9.5}{ms} less \SI{4}{ms} clears it, less \SI{2}{ms} does not. Real-time scheduling is therefore a requirement for full push margin rather than an optimization, and the loop-latency target is \SI{6}{ms} end-to-end, not \SI{10}{ms}. A second route to the same end is to move the cliff instead of the budget: doubling the sagittal velocity damping shifts the knee to \SIrange{8}{10}{ms}, which the \SI{9.5}{ms} estimate clears, at no measured cost to quiet stance (Table~\ref{tab:delay_cliff}). We report that variant without adopting it (\S\ref{sec:analysis}); a hardware build constrained to a non-RTOS budget is the case where re-qualifying it would be worth the cost. Neither scenario has been validated on hardware---latency characterization with an oscilloscope and jitter histogram is required before deployment, and it is the measurement that decides which side of the cliff a given build lands on. The discarded WBC-assist variant (\S\ref{sec:wbc}) is excluded from the hardware path on control grounds, not compute grounds: post-repair its QP solve costs \SI{0.169}{ms} mean and \SI{0.30}{ms} p99 over \num{481000} solves and would fit the budget, but admitting its torque degrades pitch by $\num{2.5}\times$ and planar drift by $\num{2.0}\times$, while gating it to the point of harmlessness also gates it to \SI{2.5}{\percent} of its permitted authority. Hardware degradation: \SI{0.294}{mm} sagittal repeatability $\rightarrow$ projected \SIrange{1}{3}{mm} (encoder quantization, backlash, stiction, tire compliance, IMU noise; cf.\ Table~\ref{tab:robustness}). Staged commissioning: static torque $\rightarrow$ standing $\rightarrow$ push recovery $\rightarrow$ flight (tethered) $\rightarrow$ terrain.

We presented ACC, a two-channel torque assembly where a PD balance core is augmented by a proximity-gated anchor integral achieving $0.294{\pm}0.015$\,mm idle repeatability on the sagittal (actuated) axis---a $59{\times}$ improvement over the proportional-only baseline under one unified protocol ($N{=}10$; Table~\ref{tab:standing})---with omnidirectional push survival ($F_{\min}{=}\SI{83}{N}$, $F_{\text{med}}{=}\SI{117}{N}$). Absolute accuracy on that axis is an order of magnitude worse: the robot parks \SI{27.0}{mm} from the commanded home, holds that offset to \SI{0.009}{mm}, and closing it is the clearest remaining work on the design.
...
survives---so exactly two pre/post deltas are quoted, both recorded in that
...
Only one failure mechanism is asserted for this baseline, the one verified
...
F4 is the one repair whose effect is partial; its residue is quantified below
alongside the \texttt{PASS}.
...
The assist's equivalence to ACC is therefore not the robust bug-independent
result an earlier draft took it for: it is a direct consequence of a gate that
is shut or nearly shut everywhere on this matrix.
...
to reduce limit cycling. The gain sweep shows that lowering $k_i$ bought no
clean-idle steadiness, so the stated reason for that move was wrong; but
\S\ref{sec:parking_ki_cost} shows it bought something the original tuning did
not measure and could not have named---margin against the noise-plus-delay
conjunction---so the setting is right for a reason other than the one
