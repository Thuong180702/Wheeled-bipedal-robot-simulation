...
\begin{table*}[t]
\centering
\caption{Idle standing precision across the full ablation ladder. All rows measured under the single protocol of Sec.~\ref{sec:standing}; values are mean${\pm}$SD over $N{=}10$ trials. See the note below the table for column definitions.}
\label{tab:standing}
...
because a \SI{20}{s} horizon flatters it. Re-running the ACC row---identical seeds, initial
conditions, profile, and metrics, with only the window lengthened---gives
...
$b_2{=}\SI{-17.3}{N}$, an order of magnitude above the between-episode
...
\begin{figure}[t]
    ...
    \SIrange{82.1}{144.1}{N}---a $1.8{\times}$ ratio between the weakest and
    ...
    (Sec.~\ref{sec:standing}). The two sagittal bearings average \SI{130.0}{N}
...
A centralized lateral task is therefore a credible direction for improving ACC's weakest axis---the \SI{98.0}{N} mean lateral push bearing of Sec.~\ref{sec:push}---but not through torque-level blending with a posture stack that cannot hold a posture.
