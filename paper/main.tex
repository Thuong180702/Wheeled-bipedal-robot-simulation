...
Estimated non-RTOS worst-case budget: sensor readout ${\sim}\SI{1}{ms}$ $+$ state estimation ${\sim}\SI{2}{ms}$ $+$ ACC computation \SI{1.27}{ms} $+$ actuation latency ${\sim}\SI{3.5}{ms}$ $\approx$ \SI{7.8}{ms} end-to-end. Three of those four terms are assumed component figures; the ACC term is measured, on the same footing as the QP solve time reported for the discarded WBC baseline (\S\ref{sec:wbc}), because a controller that reports its rejected alternative's compute cost and not its own invites the obvious question. One control step of the shipped profile costs \SI{1.11}{ms} mean and \SI{1.27}{ms} p99 over \num{5000} consecutive steps taken at the quiet-stance operating point, of which the compiled torque assembly itself is \SI{0.047}{ms} mean and \SI{0.064}{ms} p99; the remaining ${\sim}\SI{1.06}{ms}$ is the CoM and support-polygon computation together with Python-side dispatch. Two caveats bound how far that transfers. It is a JAX/XLA CPU build on a development host, and JAX would not be the deployment runtime---but the direction of that caveat is favourable, since the $23{\times}$ gap between the compiled kernel and the full call is host-side overhead that a C port removes rather than arithmetic it must reproduce. And the worst of the \num{5000} steps is \SI{1.73}{ms} rather than \SI{1.27}{ms}; that tail is scheduling jitter on a non-real-time measurement host, which is the same quantity the RTOS term below accounts for, so carrying it here as well would double-count it. What the measurement settles is that the largest term in the original estimate was assumed at ${\sim}\SI{3}{ms}$ and is at most \SI{1.3}{ms}, which moves the budget from \SI{9.5}{ms} to \SI{7.8}{ms}.\footnote{Produced by \path{scripts/measure_control_step_cost.py}; raw output \path{outputs/paper_verification/control_step_cost.json}. Host: Apple silicon (arm64), macOS, JAX CPU backend, Python 3.10.}

Read against the cliff at \SIrange{6}{8}{ms}, \SI{7.8}{ms} straddles it rather than clearly exceeding it, and two independent mitigations each move it to the clearing side. On a real-time OS with a dedicated balance-loop microcontroller (e.g., \SI{1}{kHz} RT Linux or bare-metal MCU), jitter is typically ${<}\SI{0.1}{ms}$ and \SIrange{2}{4}{ms} of budget is recoverable, putting the loop at \SIrange{3.8}{5.8}{ms}---below the cliff at either end of that range, where the \SI{9.5}{ms} figure cleared it only at the favourable end. Separately, and without touching the schedule, doubling the sagittal velocity damping moves the knee out to \SIrange{8}{10}{ms}, which \SI{7.8}{ms} clears on its own, at negligible cost to undelayed quiet stance (Table~\ref{tab:delay_cliff}). The two act on different quantities---one shortens the loop, the other moves the cliff---so they compose rather than substitute. Real-time scheduling remains the recommendation and the loop-latency target is \SI{6}{ms} end-to-end rather than \SI{10}{ms}; what the measured budget changes is that it is no longer the single thing standing between this controller and a deployable one. The damping variant moves the push cliff only---the \SIrange{14}{16}{ms} fall threshold is unchanged---so it widens the performance margin without widening the safety one, and we report it without adopting it (\S\ref{sec:analysis}); re-qualifying it over the \num{48}-scenario suite is the work a hardware build would justify. Neither scenario has been validated on hardware---latency characterization with an oscilloscope and jitter histogram is required before deployment, and it is the measurement that decides which side of the cliff a given build lands on. The discarded WBC-assist variant (\S\ref{sec:wbc}) is excluded from the hardware path on control grounds, not compute grounds: its QP solve costs \SI{0.169}{ms} mean and \SI{0.30}{ms} p99 over \num{481000} solves and would fit the budget, but admitting its torque degrades pitch by $\num{2.5}\times$ and planar drift by $\num{2.0}\times$, while gating it to the point of harmlessness also gates it to \SI{2.5}{\percent} of its permitted authority. Hardware degradation: \SI{0.294}{mm} sagittal repeatability $\rightarrow$ projected \SIrange{1}{3}{mm} (encoder quantization, backlash, stiction, tire compliance, IMU noise; cf.\ Table~\ref{tab:robustness}). Staged commissioning: static torque $\rightarrow$ standing $\rightarrow$ push recovery $\rightarrow$ flight (tethered) $\rightarrow$ terrain.
