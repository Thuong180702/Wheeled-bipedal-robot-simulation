...
defect (Appendix~\ref{sec:parking_supp}).
...
\emph{The envelope is anisotropic by construction.} The two lateral bearings average \SI{97.5}{N} against \SI{131.8}{N} for the two sagittal ones, and the weakest bearing of all is the oblique \SI{45}{\degree} ($\SI{82.8}{N}$, \SI{29}{\percent} below $F_{\text{med}}$). The asymmetry follows from the morphology rather than from tuning: the wheels can drive the contact patch back under the CoM along the sagittal axis, but produce no lateral ground force at all, so lateral recovery falls to the leg and torso alone. The oblique bearings are weakest because neither mechanism has full authority there. Raising the lateral bearing therefore requires a centralized lateral task (\S\ref{sec:wbc}), not a gain change. One feature of the sweep is not explained by this account: one oblique bearing survives \SI{40}{N} more than its mirror image, reproducibly across trials (Fig.~\ref{fig:push_polar}), and we have not traced that chirality to a mechanism.
...
Failure is consequently \emph{not} monotone in step height: a \SI{45}{\degree}
exit settles at \SI{30}{cm} and a \SI{30}{\degree} exit at \SI{40}{cm}, while both
angles fail at \SI{20}{cm}. Two regimes separate them, set by whether the leading
wheel reaches the lower ground before the trailing wheel releases---free fall over
the step takes $\sqrt{2h/g}$, comparable to the stagger interval at these heights.
A shallow step lands the leading wheel while the trailing wheel still bears on the
platform, forcing the straddle the legs cannot span; a deep step releases both
wheels together and is handled by the flight path of \S\ref{sec:flight}. Widening
the straddle regime requires knee range rather than gains. The mapping was
measured at one trial per cell over four heights and two angles on a deterministic
course, which fixes the mechanism but not the threshold; we claim no calibrated
...
A centralized lateral task is therefore a credible direction for improving ACC's weakest axis---the \SI{97.5}{N} mean lateral push bearing of Sec.~\ref{sec:push}---but not through torque-level blending with a posture stack that cannot hold a posture.
...
(2)~\textbf{Deployment timing is the binding constraint.} ACC as tuned requires a \SI{100}{Hz} loop and an actuator latency that clears the \SIrange{6}{8}{ms} push-margin cliff of Table~\ref{tab:delay_cliff}; it stops standing at all by \SIrange{14}{16}{ms}. Neither margin is comfortable: the \SI{9.5}{ms} end-to-end budget estimated below sits \emph{past} the cliff, and a \SI{50}{Hz} evaluation with coefficients recomputed for the longer step ($\alpha_a{\approx}0.58$, $\alpha_r{\approx}0.013$) collapses the robot outright (mean CoM height \SI{0.124}{m} against \SI{0.404}{m} nominal), so rate-independence is not claimed. The \SI{50}{Hz} mechanism is undetermined between excitation of plant modes the faster loop suppresses and phase lag from discretizing the asymmetric envelope at $\Delta t{=}\SI{20}{ms}$; distinguishing them needs a loop-gain and phase-margin analysis that has not been performed. Both thresholds are measured on clean sensing; how the noise$\times$delay conjunction moves them is bounded only coarsely, by the \SI{10}{ms} and \SI{30}{ms} columns of Table~\ref{tab:robustness}.
(3)~\textbf{The disturbance and terrain envelopes are characterized but their boundaries are not calibrated.} The push envelope is anisotropic for structural reasons (\S\ref{sec:push}), and one feature of it is unexplained: one oblique bearing survives \SI{40}{N} more than its mirror, reproducibly. Oblique ledge descent fails in a straddle regime rather than above a height threshold (\S\ref{sec:flight_terrain_results}), but the regime map rests on one trial per cell over four heights and two angles, which fixes the mechanism and not the boundary. Commanded height is bounded by the posture map rather than the anchor: outside the calibrated band (\SIrange{0.354}{0.454}{m} CoM-$z$) a ${+}\SI{10}{cm}$ rise collapses $F_{\max}$ by \SI{81}{\percent}, and extending the band requires more calibration points, not a controller change.
(4)~\textbf{The classical comparison is bounded by what could be linearized, and the learning comparison is not resourced.} A full-state LQR from finite-difference linearization of the 10-DOF plant produced unreliable gains at the standing equilibrium ($\kappa(A){\approx}10^{10}$), so all six baselines derive from reduced 4- and 6-state models, and a tuned convex MPC is deferred behind that same prerequisite; the hip-roll coupling $b_{r,h}{=}-5.0$ is empirical and its sensitivity unswept. The model-free PPO run (5.4M steps, one seed, no domain randomization or hyperparameter search) is a difficulty probe, not a tuned RL baseline, and its \SI{85}{\percent} fall rate is reported as observed rather than as a bound on RL for this platform; the natural comparison is residual learning over the prior rather than against it. The task-priority QP whole-body controller did not resolve the precision--recovery trade-off (\S\ref{sec:wbc}), with one caveat open: roll RMS is the single axis on which the blend beats ACC (\SI{14}{\percent}), so the centralized formulation is not without merit---only its advantage cannot be harvested by torque-level blending. Push ablations at $N{=}10$ resolve ${\sim}\SI{3.2}{N}$; sub-\SI{2}{N} differences are not significant.
