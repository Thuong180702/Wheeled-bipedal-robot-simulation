...
(4)~Push margin bifurcates at ${\sim}\SI{10}{ms}$ of actuator delay: $F_{\max}$ falls from ${\sim}\SI{95}{N}$ to ${\sim}\SI{45}{N}$ and idle RMS rises $5.5{\times}$, and the end-to-end budget estimated below leaves only \SI{0.5}{ms} of margin against that threshold. Stability is not the binding constraint---the robot survives every tested clean delay out to \SI{150}{ms} with zero falls ($N{=}5$, \S\ref{sec:analysis}), because ACC's gates key on physical state rather than on tight timing---but roughly half the usable push envelope is lost far below the rate at which it would fall.
