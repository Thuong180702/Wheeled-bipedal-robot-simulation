...
\newcommand{\tabnote}{\par\leftskip=0pt\rightskip=0pt%
...
\emph{Control-rate confound.} ACC therefore enjoys a $2{\times}$ control-bandwidth advantage over the baselines, and we were not able to remove it. Re-running ACC at \SI{50}{Hz} with the discrete coefficients recomputed for the longer step ($\alpha_a{\approx}0.58$ and $\alpha_r{\approx}0.013$, re-derived from Eq.~\eqref{eq:asym_ema} at $\Delta t{=}\SI{0.02}{s}$, with the anchor leak rescaled to \num{0.010} per step) does not preserve the controller: the robot collapses, and the mean CoM height over the recorded window is \SI{0.124}{m} against a nominal standing height of \SI{0.404}{m}. The $\SI{0.050}{mm}$ ``idle RMS'' and the \SI{10}{N} push threshold produced by that run are artifacts of a robot lying motionless on the ground---\SI{10}{N} is the lower bound of the binary search, returned when even the smallest tested impulse fails. We consequently report \emph{no} \SI{50}{Hz} ACC result and make no claim that the architecture is rate-independent. What Table~\ref{tab:lqr_baselines} establishes is that the classical baselines fail at the rate for which they were designed; it does not establish that they would fail at \SI{100}{Hz}, nor that ACC would succeed at \SI{50}{Hz}. Restoring rate parity requires re-tuning the ACC gains for the \SI{20}{ms} step---a step-response and gain-margin re-derivation we have not performed---and is stated as an open limitation (Limitation~5) rather than a resolved confound.
...
One confound in this comparison is not resolved and we state it plainly rather than argue around it: ACC runs at \SI{100}{Hz} and every baseline at \SI{50}{Hz}, so ACC enjoys a $2{\times}$ control-bandwidth advantage. We attempted to remove it by re-running ACC at \SI{50}{Hz} with the discrete EMA coefficients recomputed for the longer step, and the attempt failed---the controller does not survive the rate change, and the resulting numbers are artifacts of a fallen robot rather than a rate-independence result. The full analysis is given in \S\ref{sec:discussion} (Limitation~5); the consequence for Table~\ref{tab:lqr_baselines} is that it establishes the classical baselines fail at the rate for which they were designed, and does \emph{not} establish that they would still fail at \SI{100}{Hz}. The gap between \SI{20.0}{s} survival and \SI{0.72}{s} is far larger than a factor-of-two bandwidth change plausibly accounts for, but we have not measured that, and we do not claim it.
...
The standalone QP-WBC fails catastrophically: \textbf{\num{17.2}$\times$} more falls than ACC (5{,}985 vs.\ 347 across 225 scenarios), \textbf{\num{28.4}$\times$} more single-push falls (3{,}353 vs.\ 118), and pitch RMS \SI{41.8}{\degree} vs.\ ACC's \SI{15.9}{\degree} ($\num{2.63}\times$).\footnote{The \SI{15.9}{\degree} here is ACC's pitch RMS averaged over the mixed 225-scenario batch (including pushes and height transitions), not the clean standing value of \SI{0.060}{\degree} reported in Table~\ref{tab:lqr_baselines}.}
...
It lacks gravity-compensation feedforward, uses a single linearization at \SI{0.67}{m} base-$z$ (vs.\ the \SIrange{0.45}{0.60}{m} base-$z$ operating range of these runs, i.e.\ \SIrange{0.32}{0.47}{m} CoM-$z$; \S\ref{sec:height_convention}), and treats wheels as point contacts without a rolling constraint.
The QP solver fails on \SI{14.2}{\percent} of steps, creating torque discontinuities.
Four critical implementation bugs were identified post-hoc (contact Jacobian zero-gradient, solver feasibility-only hardcode, base-z vs.\ CoM-z height reference mismatch, and gate height-reference conflict; detailed in Appendix~\ref{sec:wbc_supp}).
...
When gates are manually overridden to force WBC contribution (keyframe test at \SI{0.53}{m} base-$z$, $\alpha_{\max}{=}0.50$, widened stability and height thresholds), the assist produces mixed results:
...
(5)~\textbf{The control-rate comparison is confounded and unresolved.} ACC runs at \SI{100}{Hz}; the LQR and PPO baselines run at \SI{50}{Hz}. A \SI{50}{Hz} ACC evaluation with recomputed discrete coefficients ($\alpha_a{\approx}0.58$, $\alpha_r{\approx}0.013$, anchor leak~\num{0.010}) \emph{failed}: the robot collapsed to a mean CoM height of \SI{0.124}{m} (nominal \SI{0.404}{m}), and the sub-millimeter idle RMS and \SI{10}{N} push threshold recorded by that run are measurements taken on a fallen robot. We therefore cannot claim that ACC's architectural contribution is rate-independent, and the $2{\times}$ bandwidth advantage over the baselines remains an unremoved confound in Table~\ref{tab:lqr_baselines}. Two observations bound how much it can explain: DT-LQR, which removes the PID servo path (the dominant lag source) and so is far less rate-sensitive, still falls within \SI{0.72}{s} with no push applied; and the failure modes differ in kind (baselines diverge in roll within one second, whereas ACC stands for the full \SI{20}{s} horizon), which a bandwidth factor alone does not obviously produce. Neither observation substitutes for a rate-matched comparison. Re-tuning ACC for the \SI{20}{ms} step, and re-running the baselines at \SI{100}{Hz}, are both required future work.
...
(8)~\textbf{A \SI{27}{mm} sagittal parking offset persists on the very axis the anchor regulates.} ACC's position integral acts on the sagittal axis, yet the robot settles $\SI{-27.0}{mm}$ from the commanded home along that axis and then holds the offset to within \SI{0.009}{mm} SD across ten trials (Table~\ref{tab:standing}). Sagittal \emph{repeatability} is excellent (\SI{0.294}{mm}), so the deficit is a static bias rather than motion, and it is not a windup artifact: it is stationary, reproducible to a hundredth of a millimeter, and largely insensitive to the gate structure (every subtractive ablation that survives lands within \SI{0.2}{mm} of it). The most likely cause is a mismatch between the latched home position and the true torque-balance equilibrium---the controller is holding the position at which the anchor integral's output balances the standing pitch bias, not the position that was commanded---compounded by the CoM-$z$ versus base-$z$ convention gap (\S\ref{sec:height_convention}). Disabling the integral moves the offset the wrong way, to $\SI{-31.2}{mm}$ (M1, Table~\ref{tab:standing}), confirming the integral removes \SI{4.2}{mm} of it but not the bulk. Absolute positioning accuracy is therefore limited to ${\sim}\SI{27}{mm}$ and is the binding constraint on any task requiring it; this is the design's principal open weakness. Calibrating the latched home against the measured equilibrium, and raising the anchor gain now that windup is gated, are the two direct routes to closing it. Both are untested.
(9)~Architectural generality across morphologies remains to be demonstrated.
(10)~Push ablation at $N{=}10$ has min.\ detectable effect ${\sim}\SI{3.2}{N}$; sub-\SI{2}{N} differences non-significant (see Statistical power). The PPO baseline (5.4M steps, single seed) may not be fully converged; the \SI{85}{\percent} fall rate is reported as observed, not as an upper bound on RL performance for this platform. A task-priority QP whole-body controller was benchmarked both standalone and as an assist on ACC and did not resolve the precision--recovery trade-off (\S\ref{sec:wbc}); re-running with known implementation bugs fixed is deferred (the architectural argument---absence of regime-gating in a fixed-task-priority formulation---does not depend on it). A tuned convex MPC on the full 10-DOF plant remains deferred because finite-difference linearization at the standing equilibrium is ill-conditioned ($\kappa(A){\approx}10^{10}$, Limitation~7), so robust full-plant MPC is an open prerequisite rather than a missing comparison.
...
\subsection{Standalone WBC (W1): Full Metric Table}
...
\footnotesize
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.33\columnwidth}ccc@{}}
\toprule
...
Posture error (ratio vs.\ ACC)                               & ---       & ---   & 8.05$\times$ \\
Yaw error (ratio vs.\ ACC)                                   & ---       & ---   & 15.43$\times$ \\
QP solver success rate                                       & \SI{85.85}{\percent} & --- & --- \\
\bottomrule
\end{tabular}
\vspace{4pt}\tabnote\noindent\footnotesize
Sources: 225-scenario batch (see \texttt{V3\_vs\_V3\_Assist\_comparison\_report.md});
Phase~3D (\texttt{full\_batch\_verdict.json});
survival time (\texttt{best\_torque\_wbc\_summary.json}).
All runs with ACC profile, no gain retuning.
WBC evaluated with known bugs (Table~\ref{tab:wbc_bugs}); W1 numbers may be pessimistic.
\end{table}

\subsection{Known WBC Implementation Bugs (F1--F4)}
...
F4 & Gate height conflict: WBC linearized at \SI{0.67}{m} base-$z$ while the gate compares against a \SIrange{0.45}{0.60}{m} base-$z$ command band. &
$g_{\text{height}} {\approx} 0$ at all test heights; WBC gate forced closed. \\
\bottomrule
\end{tabularx}
\vspace{4pt}\tabnote\noindent\footnotesize
...
\subsection{W2 Zero-Net Result Is Bug-Independent}
