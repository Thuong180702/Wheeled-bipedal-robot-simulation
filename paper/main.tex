...
\begin{figure}[t]
    ...
    plotted minimum is by construction the $F_{\min}$ of that row. It is drawn
    at $N{=}\num{10}$, the first ten seeds of that row's \num{30}; extending to
    all \num{30} moves each per-bearing mean by at most \SI{3.3}{N} and
    $F_{\min}$ by \SI{0.2}{N}, so the envelope below is plotted on the smaller
    set rather than re-rendered. Solid curve:
    ...
    (Sec.~\ref{sec:standing}). The two sagittal bearings average \SI{131.3}{N}
