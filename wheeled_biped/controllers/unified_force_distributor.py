...
class UnifiedForceDistributor:
    ...
    def __init__(
        ...
        Args:
            ...
            w_wrench: Weight for wrench tracking (soft constraint)
    ...
    def _build_cost_matrix_p(
        ...
        P = P_effort + w_wrench * A_wrench^T * A_wrench
    ...
    def distribute_wrench(
        ...
        # BoxOSQP formulation with only box constraints:
        # minimize: 0.5*x^T*P*x + q^T*x
        # subject to: l <= x <= u

        # For BoxOSQP, we need A*x = z with l <= z <= u
        # For pure box constraints, use A = I (identity)
        I = jnp.eye(8)
        A = I  # (8, 8)
        l = lower_box  # (8,)
        u = upper_box  # (8,)
        ...
        # Solve QP (BoxOSQP will handle initialization internally)
        result = solver.run(
            init_params=None,  # Let solver initialize (warm-starting requires KKTSolution)
            params_obj=(P, q),
            params_eq=A,
            params_ineq=(l, u),
        )
        ...
        # Check solver convergence (warn but don't fallback - solution should be valid)
        if hasattr(result.state, 'error') and result.state.error > 1e-2:
            import warnings
            warnings.warn(
                f"QP solver converged with moderate error ({result.state.error:.4f}). "
                f"Solution may be suboptimal but is feasible.",
                RuntimeWarning
            )
