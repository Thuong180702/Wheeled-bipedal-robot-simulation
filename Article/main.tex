...
Domain randomization is applied during training to improve robustness: body masses are perturbed by $\pm 10\%$ and ground friction coefficients by $\pm 20\%$ relative to their nominal values.
...
Observations are normalized online using Welford's running mean and variance estimator, which is updated during training but held fixed during evaluation.
...
\right],
\end{equation}
where $\rho_t(\theta)=\pi_\theta(a_t|o_t)/\pi_{\theta_{\mathrm{old}}}(a_t|o_t)$ and $\epsilon=0.2$. The full objective includes a clipped value loss (coefficient 0.5) and entropy regularization (coefficient 0.01). Advantages $\hat{A}_t$ are computed using Generalized Advantage Estimation (GAE) with $\lambda=0.95$.
...
Position drift ($r_{\mathrm{drift}}$) & Positive & 1.0 \\
...
Alive bonus ($r_{\mathrm{alive}}$) & Positive & 0.2 \\
Action rate ($p_{\Delta a}$) & Penalty & $-0.025$ \\
Wheel velocity ($p_{\mathrm{wh}}$) & Penalty & $-0.0006$ \\
Joint torque ($p_\tau$) & Penalty & $-0.0001$ \\
Joint velocity ($p_{\dot{q}}$) & Penalty & $-0.0001$ \\
\bottomrule
\end{tabular}
\end{table}
...
An episode terminates early if the torso tilt exceeds $0.8\,\mathrm{rad}$ ($\approx 46^\circ$) or the torso height drops below $0.3\,\mathrm{m}$. Otherwise, episodes run for 1000 steps (20 seconds of simulated time). A small alive bonus ($r_{\mathrm{alive}} = 1$) is given at each non-terminal step.

\section{Curriculum Learning Strategy}
\label{sec:curriculum}

\subsection{Within-Stage Height Curriculum}
Rather than sampling the commanded height from the full feasible range from the beginning, we employ a within-stage curriculum that progressively expands the lower bound of the height command distribution.

Let $\bar{r}$ denote the rolling average reward over the most recent $W$ training updates. The height lower bound is decreased by a fixed step $\delta_h$ whenever $\bar{r}$ exceeds a threshold $r_{\mathrm{thr}}$:
\begin{equation}
h_{\min}^{(k+1)} = \max\!\big(h_{\min}^{(k)} - \delta_h,\; h_{\min}^{final}\big),
\quad \text{if } \bar{r} \geq r_{\mathrm{thr}}.
\label{eq:height_curriculum}
\end{equation}

In our implementation, the initial lower bound is $h_{\min}^{init} = \SI{0.69}{m}$ (close to the nominal standing keyframe), the final lower bound is $h_{\min}^{final} = \SI{0.40}{m}$, the curriculum has $N_c = 29$ discrete levels with step size $\delta_h = (h_{\min}^{init} - h_{\min}^{final})/N_c \approx \SI{0.01}{m}$, and the threshold is set at 85\% of the maximum achievable reward. The rolling window is $W = 50$ updates. At each level, the height command is sampled uniformly:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Height curriculum uses training reward.} The within-stage height curriculum advances based on rolling training reward rather than held-out evaluation metrics. This is noisier and could lead to premature curriculum advancement. The multi-stage curriculum uses proper evaluation gating.
\end{enumerate}
